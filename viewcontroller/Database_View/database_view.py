...
class DatabaseView:
    def __init__(self, parent, language, my_gps_data_list):
        ...
        # ===== Controls =====
        self.show_unsaved_cb = tk.Checkbutton(
            self.frame,
            variable=self.show_unsaved_var,
            command=self.refresh_table
        )
        self.show_unsaved_cb.pack(anchor="w")

        self.enable_color_cb = tk.Checkbutton(
            self.frame,
            variable=self.enable_color_var,
            command=self.refresh_table
        )
        self.enable_color_cb.pack(anchor="w")
    ...
    def refresh_texts(self):
        self.show_unsaved_cb.config(
            text=self.lang.t("database_view", "show_unsaved")
        )
        self.enable_color_cb.config(
            text=self.lang.t("database_view", "enable_color")
        )
        self.delete_button.config(
            text=self.lang.t("database_view", "delete_selected")
        )

        headers = self.lang.t("database_view", "headers")
    ...
    def on_header_click(self, col):
        ...
        symbol = "☑" if self.header_select_all else "☐"
        self.tree.heading("selected", text=symbol)
